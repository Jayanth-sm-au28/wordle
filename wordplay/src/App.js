...
import './App.css';
import Keyboard from './entities/virtualKeyboard.js';
import { wordList } from './constants/dictionary.js';
...
const App = () => {
  ...
  const enterBoardWord =(word) =>{
    ...
    if(matchCount === 5){
      status="WIN";
      handleMessage("CONGRULATIONS YOU WON")
    }
    ...
    if(key==="ENTER"){
        if(charArray.length===5){
            ...
            if(!wordList[word.charAt(0)].includes(word)) {
                handleError();
                handleMessage("INVALID WORD TRY AGAIN");
                return;
              }
            enterBoardWord(word);
            setCharArray([]);
        }else{
            handleMessage("ENTER COMPLETE ROW");
        }
        return;
    }
    if(key==="⌫"){
